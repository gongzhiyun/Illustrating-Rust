# 图解 Rust 字符串：内存模型与 UTF-8 权衡

在 Rust 中，字符串并不是一种简单的基本类型。为了在内存安全、运行效率和国际化支持（UTF-8）之间取得平衡，Rust 设计了多种字符串形态。理解这些形态的底层差异，是写好 Rust 代码的第一步。
...
Rust 字符串的设计并没有试图掩盖底层内存的复杂性，而是通过所有权系统将这种复杂性暴露给开发者。**String 负责管理生命周期和堆内存，而 &str 则提供了高效观察内存的窗口。** 只有理解了这种“权衡”，我们才能在享受 UTF-8 带来的国际化支持时，依然写出高性能的系统级代码。
