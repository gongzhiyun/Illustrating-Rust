...
- **静态分发（泛型）**：
    ...
    - **快如闪电**：因为代码是现成的，编译器可以直接把函数调用“拆掉”，直接把逻辑插进去（内联）。这就是 Rust 所谓“零成本抽象”的关键。
- **动态分发（特征对象）**：
...
Rust 的泛型先通过**特征约束**提要求，再通过**单态化**自动复印代码，最后配合**内存布局优化**把数据压实。这种“编译期解决一切”的做法，让你在写高大上的通用代码时，依然能拥有极高的运行效率。
