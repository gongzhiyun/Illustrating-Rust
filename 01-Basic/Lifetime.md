...
> 为什么编译器总是抱怨“引用存活时间不够长”？
> 生命周期标注真的是在“改变”变量的寿命吗？还是仅仅在向编译器“证明”某种安全事实？

生命周期是 Rust 内存安全的关键支柱之一，它的任务只有一个：**确保所有引用永远有效**。
...
当结构体持有引用时，必须显式标注生命周期。这相当于向编译器做出一项**显式保证**：结构体本身的寿命绝对不会超过它所持有的引用的寿命。
...
3.  **显式标注的作用**：
    通过标注 `<'a>`，你实际上是在告诉编译器：
103→    - “返回值引用的生命周期，将与 `x` 和 `y` 中**寿命较短**的那一个保持一致。”
104→    - 这建立了一个**通用的约束**。调用方在使用该函数时，借用检查器会根据这个约束来验证返回值的引用是否超出了任何一个潜在源头的作用域。
...
**生命周期标注不是为了延长变量的寿命，而是为了向编译器提供引用的有效性证据，从而换取运行时的零成本安全。**
