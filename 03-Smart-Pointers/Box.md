# Rust Box：为什么递归类型必须包裹在堆内存里？

> 为什么一个简单的递归枚举（如 `List`）会导致编译器报错“尺寸无限”？
> `Box<T>` 仅仅是为了把数据扔进堆里吗？它在实现“零成本抽象”中扮演了什么角色？
...
- [5. 性能表现：效率与灵活性的抉择](#5-性能表现效率与灵活性的抉择)
- [6. 总结](#6-总结)
...
`Box<T>` 是 Rust 中最基础的智能指针。它不包含引用计数或复杂的运行时检查，其主要功能非常明确：**在栈上存储一个固定大小的地址，而将数据本体保存在堆中。**
...
在理解了 `Box` 的内存布局和各种应用场景后，我们必须回到一个关键问题：在实际开发中，我们应该在什么时候、以什么样的代价去使用它？
