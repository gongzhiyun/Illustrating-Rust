# Rust Rc：如何让一个值拥有多个“主人”？

> 在 Rust 的严苛规则下，一个值只能有一个所有者。那么当我们需要构建复杂的图形结构或 UI 组件时，该如何打破这种“一夫一妻制”？
> `Rc<T>` 是如何通过内存中的一小块计数器，优雅地实现堆内存共享的？
...
> **基本原则**：`Rc<T>` 仅适用于**单线程**场景。如果需要跨线程共享，请使用 `Arc<T>`。
...
`Rc<T>` 通过引用计数在堆上实现了单线程下的所有权共享。它在保持高性能的同时，为构建复杂的数据结构提供了必要的灵活性。
